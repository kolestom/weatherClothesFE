...
export const Admin = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("token")) {
      ...
      const init = async () => {
        try {
          const response = await client.get("/api/pref", {
            headers: {
      ...
      <div className={styles.prefContainer}>
        {prefs ? (
          <Select value={selectedOption} placeholder="Select a preference to update" onChange={handlePrefUpdate}>
            {prefs.map((pref) => (
              <option value={pref._id} key={pref._id}>
                {pref.prefName}, min: {pref.minTemp} °C, max: {pref.maxTemp} °C
              </option>
            ))}
          </Select>
        ) : (
        ...
        {selectedPref && (
          <PrefCardAdmin {...{ selectedPref, setPrefs, isOpen, onClose }} />
        )}
      </div>
      <PrefCreate {...{ setPrefs }} />
    </div>
